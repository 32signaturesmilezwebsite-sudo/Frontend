...
const AdminBlogEdit = () => {
  ...
  const existingThumbnailUrl = existingThumbnail
    ? (existingThumbnail.startsWith('http') ? existingThumbnail : `${import.meta.env.VITE_API_URL}${existingThumbnail}`)
    : null;
    ...
    <div className="ag-layout">
      ...
      <header className="ag-header">
        <div className="ag-header-brand">
          ...
          <span>Admin Panel – Edit Blog Post</span>
        </div>
      ...
      <div className="ag-content" style={{ maxWidth: '960px' }}>
        ...
        {fetchLoading ? (
          <div style={{ textAlign: 'center', padding: '80px', color: '#6b7280' }}>Loading blog post…</div>
        ) : (
          <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e5e7eb', boxShadow: '0 4px 24px rgba(59,70,87,0.07)', padding: '36px' }}>
            ...
            <div style={{ marginBottom: '28px', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid #f3f4f6', paddingBottom: '20px' }}>
              ...
              <span style={{ marginLeft: 'auto', padding: '4px 12px', background: 'rgba(197,123,67,0.1)', color: '#c57b43', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 600 }}>
                Editing Mode
              </span>
            </div>
            ...
            {success && (
              <div style={{ padding: '12px 16px', background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: '8px', color: '#16a34a', marginBottom: '24px', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="ag-form-group">
                <label className="ag-label">Excerpt <span style={{ color: '#9ca3af', fontSize: '0.8rem', fontWeight: 400 }}>– shown in blog list cards</span></label>
                <textarea value={excerpt} onChange={e => setExcerpt(e.target.value)} placeholder="A short 1-2 sentence summary…" rows={2} className="ag-input" style={{ resize: 'vertical' }} />
              </div>
              ...
              <div className="ag-form-group">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
                  <label style={{
                    ...
                    cursor: 'pointer', background: 'rgba(197,123,67,0.04)', color: '#c57b43', flexShrink: 0
                  }}>
                    <ImageIcon size={28} style={{ marginBottom: '8px' }} />
                    <span style={{ fontSize: '0.82rem', fontWeight: 600 }}>Replace Image</span>
                    <input type="file" accept="image/*" onChange={handleImageChange} style={{ display: 'none' }} />
                  </label>

                  {/* Existing thumbnail (show if no new one selected) */}
                  {!thumbnailPreview && existingThumbnailUrl && (
                    <div>
                      <p style={{ fontSize: '0.8rem', color: '#9ca3af', margin: '0 0 6px' }}>Current thumbnail:</p>
                      <img src={existingThumbnailUrl} alt="Current" style={{ width: '200px', height: '120px', objectFit: 'cover', borderRadius: '10px', border: '1px solid #e5e7eb' }} />
                    </div>
                  )}
                  ...
                  {thumbnailPreview && (
                    <div style={{ position: 'relative' }}>
                      <p style={{ fontSize: '0.8rem', color: '#9ca3af', margin: '0 0 6px' }}>New thumbnail:</p>
                      <img src={thumbnailPreview} alt="New Preview" style={{ width: '200px', height: '120px', objectFit: 'cover', borderRadius: '10px', border: '1px solid #e5e7eb' }} />
                      <button type="button" onClick={clearNewThumbnail} style={{ position: 'absolute', top: '18px', right: '-8px', background: '#ef4444', border: 'none', borderRadius: '50%', width: '24px', height: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                        <X size={12} />
                      </button>
                    </div>
                  )}
              ...
              <div className="ag-form-group">
                ...
                <div style={{ border: '1px solid #d1d5db', borderRadius: '8px', overflow: 'hidden' }}>
                  {/* Only render Jodit when content is loaded to prevent blank editor */}
                  {content !== undefined && (
                    <JoditEditor
                      ref={editor}
                      value={content}
